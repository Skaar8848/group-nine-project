...
**关键节点**：
- 高被引Top3：Gautschi M(2017, 1256次)、de Oliveira AB(2020, 987次)、Fritzmann T(2020, 865次)
- 中介中心性Top3：Shacham H(2007, 0.123)、Lipp M(2018, 0.098)、Alglave J(2014, 0.085)

**主要发现**：
1. RISC-V领域已形成9个主要知识聚类
2. 领域发展经历早期基础→技术成熟→前沿应用三个阶段
3. 高被引文献构成领域知识基础，中介中心性文献是跨领域桥梁
4. 当前热点：AI加速、边缘计算、安全性

**局限性**：
- 数据来源偏差（仅Web of Science）
- 共被引分析偏向高被引文献
- 聚类解释存在主观性

**交付文件**：
- outputs/图1_RISC-V领域共被引网络图.png（300DPI）
- outputs/共被引网络分析报告.md（含≥200字解释）
- outputs/关键指标.txt（记录所有指标）
...
优化检索式后重新完成了lesson3以及lesson4的内容，输出结果，新脚本新数据放在（updated outputs by lesson3and4）里面。
